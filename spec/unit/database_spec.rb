...
describe CouchPotato::Database, 'bulk_save' do
  ...
  it "should save all new documents" do
    c1 = Category.new(name: "pizza")
    expect(@db.couchrest_database).to receive(:bulk_save).with([c1])
    @db.bulk_save([c1], false)
  end

  it "should only update dirty documents" do
    c1 = Category.new(_rev: "1-abc")
    c2 = Category.new(_rev: "1-def")
    c2.is_dirty
